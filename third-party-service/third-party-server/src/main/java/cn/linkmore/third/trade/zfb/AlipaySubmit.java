...
public class AlipaySubmit {
	...
	public static String buildRequestMysign(Map<String, String> sPara,String key) {
        ...
        if(AlipayConfig.sign_type.equals("MD5") ) {
        	mysign = MD5.sign(prestr, AlipayConfig.key(key), AlipayConfig.input_charset);
        }
        return mysign;
    }
    ...
    public static String buildRequest(Map<String, String> sParaTemp, String strMethod, String strButtonName,String key) {
        ...
        sbHtml.append("<input type=\"submit\" value=\"" + strButtonName + "\" style=\"display:visable;\"></form>");
        sbHtml.append("<script>/*document.forms['alipaysubmit'].submit();*/</script>");

        return sbHtml.toString();
    }
    
    public static String buildRequest(Map<String, String> sParaTemp, String strMethod,String key) {
    	 //待请求参数数组
        Map<String, String> sPara = buildRequestPara(sParaTemp,key);
        StringBuffer parameters = new StringBuffer();
		parameters.append(ALIPAY_GATEWAY_NEW).
		...
		for (int i = 0; i < keys.size(); i++) {
            String name = (String) keys.get(i);
            String value = (String) sPara.get(name);
            parameters.append(name).append("=").append(value).append(i==keys.size()-1?"":"&");
